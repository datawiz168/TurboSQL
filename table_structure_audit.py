...
def audit_table_structure(conn, tables_in_query):
    ...
    query29 = """
     ...
     WHERE c.system_type_id = 34  -- 34 是 SQL Server 中 IMAGE 数据类型的 ID，用于存储 BLOB 数据
     """
    ...
    SELECT table_name, column_name
    FROM information_schema.columns
    WHERE table_schema = 'dbo' -- 根据实际情况调整schema名称
    """
    ...
    for table in empty_tables:
        print(f"警告: 表 {table.TableName} 是空的。")
    ...
    # 为 tables_in_query 列表中的每个表名创建一个问号（?）占位符
    placeholders = ', '.join('?' for _ in tables_in_query)
    query101 = query101.format(placeholders)
    cursor.execute(query101, tables_in_query)
